...
def generate_launch_description():
  ...
  cob_slam_robot_ros1_bridge_config = os.path.join(
  ...
  load_bridge_params = ExecuteProcess(
      cmd=['rosparam', 'load', cob_slam_robot_ros1_bridge_config]
  )

  ros1_topic_bridge_parameter_bridge = ExecuteProcess(
      cmd=['ros2', 'run', 'ros1_bridge', 'parameter_bridge', '__ns:=bridge_cob_slam_robot_topics', '__name:=ros1_topic_bridge_parameter_bridge']
  )
