...
def generate_launch_description():
  ...
  cob_combi_robot_uml_ros1_bridge_config = os.path.join(
    get_package_share_directory('cob_combi_robot_uml'),
    'config',
    'ros1_bridges.yaml'
  )

  load_bridge_params = ExecuteProcess(
      cmd=['rosparam', 'load', cob_combi_robot_uml_ros1_bridge_config]
  )

  ros1_topic_bridge_parameter_bridge = ExecuteProcess(
      cmd=['ros2', 'run', 'ros1_bridge', 'parameter_bridge', '__ns:=bridge_cob_combi_robot_uml_topics', '__name:=ros1_topic_bridge_parameter_bridge']
  )
  ros1_service_from_bridge_parameter_bridge = ExecuteProcess(
      cmd=['ros2', 'run', 'ros1_bridge', 'parameter_bridge', '__ns:=bridge_cob_combi_robot_uml_from_services', '__name:=ros1_service_from_bridge_parameter_bridge']
  )
