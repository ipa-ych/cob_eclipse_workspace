...
def generate_launch_description():
  ...
  cob_bridge_ros1_bridge_config = os.path.join(
    get_package_share_directory('cob_bridge'),
    'config',
    'ros1_bridges.yaml'
  )

  load_bridge_params = ExecuteProcess(
      cmd=['rosparam', 'load', cob_bridge_ros1_bridge_config]
  )

  ros1_topic_bridge_parameter_bridge = ExecuteProcess(
      cmd=['ros2', 'run', 'ros1_bridge', 'parameter_bridge', '__ns:=bridge_cob_bridge_topics', '__name:=ros1_topic_bridge_parameter_bridge']
  )
